...
echo "<td><a href='userManagement.php?id=$email#popupLogin' data-rel='popup' data-overlay-theme='a' data-position-to='window' data-transition='slide' class='ui-btn ui-corner-all ui-shadow ui-icon-alert ui-btn-icon-notext' style='margin-left:50px;'></a></td>";
  echo "</tr>";
 
}
	if (isset($_GET['submit'])) {
		if(!preg_match('/^(?=.*\d)(?=.*[A-Za-z])[0-9A-Za-z]{8,20}$/', $_GET['pass'])) {
			...
			header("Refresh: 5; url=userManagement.php");
				...
				update($users,$_GET['id'],"",$_GET['pass']);
    ...
    <form>
        <div style="padding:10px 20px;">
            <h3>Please Enter a new password</h3>
			 <input name="id" id="email" value="<?php if(isset($_GET['id'])){echo"$_GET['id']";}?>" placeholder="New password" data-theme="b" type="hidden"  >
            <label for="pw" class="ui-hidden-accessible">Password:</label>
            <input name="pass" id="pw" value="" placeholder="New password" data-theme="b" type="password" required >
			<button aria-disabled="false" class="ui-btn-hidden" type="submit" name="submit" value="submit" >Confirm</button></div>
        </div>
    </form>
</div>
</div>
